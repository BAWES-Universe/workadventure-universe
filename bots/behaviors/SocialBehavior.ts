                ...
                if (chunk.done) {
                    ...
                    if (this.responseProcessor && fullMessage.trim()) {
                        ...
                        if (processed.metrics.repetitionScore >= 1.0 && processed.issues.includes('BLOCKED: Exact duplicate')) {
                            if (process.env.NODE_ENV === 'development' || process.env.ENABLE_BOT_DEBUG === 'true') {
                                console.warn(`[SocialBehavior] ⚠️ Exact duplicate detected for bot ${botId}, player ${playerId}. Blocking response: "${fullMessage.substring(0, 50)}..."`);
                            }
                            ...
                            try {
                                ...
                                if (regeneratedMessage.trim() && this.responseProcessor) {
                                    ...
                                    if (process.env.NODE_ENV === 'development' || process.env.ENABLE_BOT_DEBUG === 'true') {
                                        console.log(`[SocialBehavior] ✅ Regenerated response after blocking duplicate`);
                                    }
                                } else {
