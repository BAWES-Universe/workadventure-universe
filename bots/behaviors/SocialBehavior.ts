                ...
                if (chunk.done) {
                    ...
                    if (this.responseProcessor && fullMessage.trim()) {
                        ...
                        while (currentRepetitionScore >= 1.0 && processed.issues.includes('BLOCKED: Exact duplicate') && regenerationAttempts < maxRegenerationAttempts) {
                            ...
                            if (process.env.NODE_ENV === 'development' || process.env.ENABLE_BOT_DEBUG === 'true') {
                                console.warn(`[SocialBehavior] ⚠️ Exact duplicate detected for bot ${botId}, player ${playerId} (attempt ${regenerationAttempts}/${maxRegenerationAttempts}). Blocking response: "${currentMessage.substring(0, 50)}..."`);
                            }
                        ...
                        if (currentRepetitionScore >= 1.0 && regenerationAttempts >= maxRegenerationAttempts) {
                            console.warn(`[SocialBehavior] ⚠️ Still duplicate after ${maxRegenerationAttempts} attempts, using fallback`);
                            processedMessage = "Let me think about that differently...";
                        }
