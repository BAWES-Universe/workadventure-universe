...
if (shouldRespond && !this.bot.getState().isMoving() && !this.bot.getIsFollowingPath() && 
    ...
    protected onMemoryReady(spaceName: string, user: SpaceUser & { id: number }): void {
        ...
        if (this.justCompletedLeading && this.justCompletedLeading.targetPersonId === user.id) {
            if (process.env.NODE_ENV === 'development' || process.env.ENABLE_BOT_DEBUG === 'true') {
                console.log(`[PatrolBehavior] onMemoryReady: deferring to onSpaceJoined for player ${user.id} — leading-completion greeting pending`);
            }
            return;
        }
            ...
            for await (const chunk of this.aiService.generateBotResponseStream(
                ...
                if (chunk.done) {
                    ...
                    if (parsedResponse.emotions && this.conversationMemory) {
                        ...
                        this.conversationMemory.updateEmotionsFromAI(botId, followerPlayer.userId, {
                            personSentiment: 0,
                            isInsult: false,
                            insultSeverity: 0,
                            context: 'neutral',
    ...
    protected override canSendGreetingToPlayer(spaceName: string, _playerId: number): boolean {
        return Boolean(this.bot && this.currentSpaceName === spaceName);
    }
     ...
     */
    protected override async sendGoodbyeAndReturn(spaceName: string, playerId: number, botId: string, destinationType: 'person' | 'area'): Promise<void> {
        if (!this.bot || !this.aiService) {
            this.returnAfterLeading();
            return;
        }
