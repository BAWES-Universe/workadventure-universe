...
The person you're talking to just sent several more messages while you were still finishing a response. Acknowledge this in one short sentence, in your own voice — you've noticed and will get to them.`;
    ...
    /**
     * Hook to determine whether a greeting response bubble should be sent to the player.
     * Default allows sending whenever the bot client is connected.
     */
    protected canSendGreetingToPlayer(_spaceName: string, _playerId: number): boolean {
        return Boolean(this.bot);
    }

    /**
     * Hook returning default chat instructions when none are configured on the bot.
     */
    protected getDefaultGreetingInstructions(): string {
        return 'You are a friendly bot. Respond naturally when someone approaches you.';
    }

    /**
     * Hook returning the logging prefix for debug statements.
     */
    ...
    /**
     * Subclasses implement sendGoodbyeAndReturn for area/person departures.
     */
    protected abstract sendGoodbyeAndReturn(
        spaceName: string,
        playerId: number,
        botId: string,
    ...
    /**
     * Generate AI greeting when leading someone or a group to a target person.
     */
    protected async generateAIGreetingWithLeadingContext(
        spaceName: string,
        playerId: number,
        botId: string,
        ...
        try {
            ...
            for await (const chunk of this.aiService.generateBotResponseStream(
                ...
                playerId,
                playerMessage,
                botConfig.chatInstructions || this.getDefaultGreetingInstructions(),
                ...
                if (chunk.done) {
                    ...
                    if (parsedResponse.emotions && this.conversationMemory) {
                        ...
                        if (process.env.NODE_ENV === 'development' || process.env.ENABLE_BOT_DEBUG === 'true') {
                            console.log(`${this.getLogPrefix()} AI omitted emotion block, using neutral fallback`);
                        }
                    ...
                    if (this.responseProcessor && cleanedMessage.trim()) {
                        const chatInstructions = botConfig.chatInstructions || this.getDefaultGreetingInstructions();
                    ...
                    if (cleanedMessage.trim()) {
                        if (this.canSendGreetingToPlayer(spaceName, playerId)) {
                    ...
                    this.sendGoodbyeAndReturn(spaceName, playerId, botId, 'person').catch(error => {
                        console.error(`${this.getLogPrefix()} Error sending goodbye and returning:`, error);
                    });
                    break;
                }
            }
        } catch (error) {
            console.error(`${this.getLogPrefix()} AI leading completion greeting error:`, error);
            this.bot?.sendStreamMessage(spaceName, greetingResponseId, '', true, '');
        } finally {
    ...
    /**
     * Generate AI greeting for a person approaching or being approached
     */
    protected async generateAIGreeting(
        spaceName: string,
        playerId: number,
        botId: string
    ): Promise<void> {
        ...
        try {
            ...
            if (process.env.NODE_ENV === 'development' || process.env.ENABLE_BOT_DEBUG === 'true') {
                console.log(`${this.getLogPrefix()} generateAIGreeting: context.length=${context.length}, hasContext=${hasContext}`);
                if (hasContext) {
                    console.log(`${this.getLogPrefix()} Context preview: ${context.substring(0, 200)}...`);
                }
            }
            ...
            console.error(`${this.getLogPrefix()} AI greeting error:`, error);
            this.bot?.sendStreamMessage(spaceName, greetingResponseId, '', true, '');
        } finally {
            // Stop typing indicator regardless of how the stream ended
            this.bot?.stopTyping(spaceName);
